...
public class RabbitConsumer {
    ...
    public static void main(String[] argv) throws Exception {
        ...
        // Declare exchange + queue, then bind
        channel.exchangeDeclare(exchangeName, BuiltinExchangeType.FANOUT, true);
        channel.queueDeclare(queueName, true, false, false, null);
        channel.queueBind(queueName, exchangeName, "");

        String timestampSubscribe = LocalDateTime.now().format(dtf);

        System.out.println(timestampSubscribe + " [*] Subscribed to exchange: " + exchangeName + " via queue: " + queueName);
        ...
        DeliverCallback deliverCallback = (consumerTag, delivery) -> {
            ...
            try (FileWriter fw = new FileWriter(logFilePath, true);
                ...
                System.err.println(timestamp + " Failed to write message to log file: " + e.getMessage());
                System.out.println(timestamp + " Failed to write message to log file: " + e.getMessage());
            }
        };

        // Add a custom consumer tag so it appears in RabbitMQ UI
        String consumerTag = "docker-consumer-" + System.currentTimeMillis();
        channel.basicConsume(queueName, true, consumerTag, deliverCallback, consumerTag1 -> {});

        // Block forever so the JVM doesn’t exit
